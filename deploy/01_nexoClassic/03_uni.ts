...
async function deployment(hre: HardhatRuntimeEnvironment): Promise<void> {
  ...
  if (network.name == "ropsten" || network.name == "mainnet") {
    ...
    const NEXO = (await ethers.getContract("NEXO")) as Erc20Mock
    const tokens = [WETH.address, WBTC.address, USDC.address, NEXO.address]
    ...
    await NEXO.mint(ethers.utils.parseUnits("5000000", 18))

    for (let i = 0; i < tokens.length; i++)
      for (let j = 0; j < i; j++) {
        if (!amounts[tokens[i]] || !amounts[tokens[i]][tokens[j]]) continue
