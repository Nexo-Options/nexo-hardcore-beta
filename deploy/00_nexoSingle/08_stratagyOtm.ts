...
async function deployment(hre: HardhatRuntimeEnvironment): Promise<void> {
  ...
  const Pool = await get("NexoOperationalTreasury")
  ...
  const paramsBTC = {
    ...
    spotDecimals: 8,
    IVRates: [BN.from("0"), BN.from("0"), BN.from("0"), BN.from("0")],
    borders: [11, 20, 30],
    roundedDecimals: 11,
    percent: 110,
    limit: 1000000e6,
  ...
  async function deployOTM(params: typeof paramsETH) {
    ...
    const strategyName = `NexoStrategyOTM_${type}_${params.percent}_${params.currency}`
    ...
    const strategy = await deploy(strategyName, {
      contract: params.percent > 100 ? "NexoStrategyCall" : "NexoStrategyPut",
      from: deployer,
      log: true,
      args: [
        params.pool,
  ...
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("13000"), BN.from("22000"), BN.from("25500"), BN.from("35000")],
    percent: 70,
  })
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("29500"), BN.from("39000"), BN.from("45000"), BN.from("56000")],
    percent: 80,
  })
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("77000"), BN.from("90000"), BN.from("105000"), BN.from("110000")],
    percent: 90,
  })
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("60000"), BN.from("80000"), BN.from("95000"), BN.from("105000")],
    percent: 110,
  })
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("19000"), BN.from("32000"), BN.from("45000"), BN.from("50000")],
    percent: 120,
  })
  await deployOTM({
    ...paramsETH,
    IVRates: [BN.from("12000"), BN.from("19000"), BN.from("25000"), BN.from("37000")],
    percent: 130,
  })
