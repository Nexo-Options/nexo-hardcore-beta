...
describe("NexoBondingCurve", async () => {
  let USDC: ERC20
  let NEXO: ERC20
  let deployer: Signer
  let alice: Signer
  ...
  beforeEach(async () => {
    ...
    await NEXO.connect(alice).approve(
    ...
    await NEXO.mintTo(
      EthBondingCurve.address,
      ethers.utils.parseUnits("100000000", await NEXO.decimals()),
    )
  ...
  describe("ETH Bonding Curve", () => {
    ...
    it("Buy NEXO", async () => {
      ...
      let startBalanceBC = await NEXO.balanceOf(EthBondingCurve.address)
      ...
      expect(ethers.utils.formatUnits(startBalanceBC.toString())).to.be.eq(
        ethers.utils.formatUnits(
          (await NEXO.balanceOf(EthBondingCurve.address))
            .add(
              startBalanceBC.sub(
                await NEXO.balanceOf(EthBondingCurve.address),
              ),
    ...
    it("Sell NEXO", async () => {
      let startBalanceBCBeforeBuy = await NEXO.balanceOf(
        EthBondingCurve.address,
      )
      let amountTokens = 100000
        ...
        ethers.utils.formatUnits(
          (
            await ethers.provider.getBalance(
              await EthBondingCurve.nexoDevelopmentFund(),
            )
          ).toString(),
        ),
      ).to.be.eq(prc10)
    ...
    it("Nexo Amount need higher then 0", async () => {
      await EthBondingCurve.connect(alice).buy(
      ...
      ).to.be.revertedWith("Nexo Amount need higher then 0")
    })
    ...
    it("Buy and Sell NEXO in one transaction")
  })
})
