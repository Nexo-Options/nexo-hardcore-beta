...
import {deployParams} from "../../deploy/01_NexoClassic/07_AdaptivePricers"
...
describe("PriceCalculator", async () => {
  ...
  beforeEach(async () => {
    ...
    const SELLER_ROLE = await nexoPoolWETH.SELLER_ROLE()
    nexoPoolWETH.grantRole(SELLER_ROLE, await alice.getAddress())
  ...
  describe("_priceModifier", () => {
    const prices: number[] = []
    beforeEach(async () => {
      await nexoPoolWETH.setMaxUtilizationRate(100)
    })
    for (let utilized = 0; utilized < 100; utilized += 10)
      it(`test1 ${utilized}%`, async () => {
        ...
        const x = await nexoPoolWETH.calculateTotalPremium(
          24 * 3600,
