...
async function main() {
  ...
  const fixture = deployments.createFixture(async ({ deployments }) => {
    ...
    return {
      deployer,
      alice,
      nexoOtmCallBTC: (await ethers.getContract(
  ...
  const { alice, deployer, nexoOtmCallBTC, WBTC, USDC, OtmPriceCalculator_BTC, btcPriceFeed } = contracts
  await nexoOtmCallBTC.setLimit(
