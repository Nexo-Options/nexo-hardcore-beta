...
async function main() {
  ...
  const fixture = deployments.createFixture(async ({ deployments }) => {
    ...
    return {
      deployer,
      alice,
      nexoOtmPutBTC: (await ethers.getContract(
  ...
  const { alice, deployer, nexoOtmPutBTC, WBTC, USDC, OtmPriceCalculator_BTC, btcPriceFeed } = contracts
  await nexoOtmPutBTC.setLimit(
