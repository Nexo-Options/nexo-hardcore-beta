...
async function main() {
  ...
  const fixture = deployments.createFixture(async ({ deployments }) => {
    ...
    return {
      deployer,
      alice,
      nexoOtmCallETH: (await ethers.getContract(
  ...
  const { alice, deployer, nexoOtmCallETH, WETH, USDC, OtmPriceCalculator_ETH, ethPriceFeed } = contracts
  await nexoOtmCallETH.setLimit(
